...
document.querySelectorAll('[data-preset]').forEach(button => {
  button.addEventListener('click', () => {
    ...
    if (preset === 'default' || preset === 'enumeration-only' || preset === 'gray-box' || isFullAuthorized || isAuthDebug) form.elements.profile.value = 'auto';
    if (isAuthDebug) form.elements.profile.value = 'crapi';
    form.elements.mode.value = preset === 'gray-box' || isFullAuthorized ? 'gray-box' : 'black-box';
    if (isAuthDebug) form.elements.mode.value = 'gray-box';
    form.elements.account_source.value = isFullAuthorized || preset === 'gray-box' ? 'generated_test_accounts' : 'auto';
    if (isAuthDebug) form.elements.account_source.value = 'generated_test_accounts';
    form.elements.auth_mode.value = 'auto';
    form.elements.auth_mode.value = form.elements.account_source.value;
    form.elements.execution_profile.value = isFullAuthorized ? 'destructive-full-scan' : (preset === 'gray-box' ? 'authenticated-full-scan' : 'discovery_only');
    if (isAuthDebug) form.elements.execution_profile.value = 'auth-authorization-debug';
