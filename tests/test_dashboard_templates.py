...
class DashboardTemplateTests(unittest.TestCase):
    ...
    def test_control_page_separates_application_and_execution_profiles(self) -> None:
        ...
        self.assertIn("This does not decide how aggressive the scan is.", response.text)
        self.assertIn("Select how deep SAIF should test.", response.text)
        self.assertIn("Generic Web/API", response.text)
        self.assertIn("OWASP Juice Shop application profile", response.text)
        self.assertIn("DVWA application profile", response.text)
        self.assertIn("Custom application profile", response.text)
        self.assertIn("Safe Enumeration", response.text)
        self.assertIn("Standard Non-Destructive Scan", response.text)
        self.assertIn("Authenticated Full Scan", response.text)
        self.assertIn("Effective Scan Configuration", response.text)
        self.assertIn("Lab full allowed", response.text)
        self.assertIn("target-agnostic", response.text)
    ...
    def test_destructive_full_preset_sets_effective_scan_payload(self) -> None:
        ...
        self.assertIn("syncExecutionProfileFlags(form)", script)
        self.assertIn("standard_non_destructive", script)
        self.assertIn("authenticated_full", script)
